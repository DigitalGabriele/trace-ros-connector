...
TRACE_CONNECTOR(trace::blueye_ros_connector::Connector,
                "trace_blueye_ros_connector", "0.0.0");

namespace trace {
namespace blueye_ros_connector {

Connector::Connector() : Connector("trace_blueye_ros_connector") {
...
void Connector::initialize() {
  RCLCPP_ERROR(this->get_logger(), "Connector::initialize() CALLED");
...
bool Connector::check_resource(const std::string &resource_name, const PropertyMap &properties) {
  ...
  std::string clean = trim(resource_name);

  RCLCPP_ERROR(this->get_logger(),
               "check_resource raw='%s' clean='%s'",
               resource_name.c_str(), clean.c_str());

  return (this->resource_to_client_map.find(clean) != this->resource_to_client_map.end());
}

bool Connector::reserve_resource(const std::string &task_uuid, const std::unordered_map<std::string, std::string> &properties) {
  auto resource_name = trim(properties.at("resource_name"));
